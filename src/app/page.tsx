    ...
    <div className="flex flex-col min-h-screen p-6 bg-gray-100">
      ...
      <main className="text-black flex-grow flex flex-col items-center">
        ...
        <section className='w-full max-w-3xl'>
          <div>
            {/* OTC Section */}
            <h3 className="text-2xl text-black font-semibold mb-4">Top 3 일반 의약품(OTC) 매출액</h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
              {categoriesOTC.map(category => (
                <div key={category.code} onClick={()=>router.push(`/categories/${category.code}`)} className="bg-white rounded-md shadow transition-transform transform hover:scale-105 hover:shadow-lg">
                  <div className="flex flex-col items-center justify-center p-4">
                    <p className="text-gray-600">{category.code}</p>
                    <h4 className="text-lg text-black font-semibold">{category.name}</h4>
                  </div>
                  <img src={category.image} alt={category.name} className="w-full h-32 rounded-t-md object-scale-down object-center" />
                </div>
              ))}
            </div>

            {/* ETC Section */}
            <h3 className="text-2xl text-black font-semibold mb-4">Top 3 전문의약품(ETC) 매출액</h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
